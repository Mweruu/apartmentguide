...
<head>
    ...
    <style>
		...
		#cityList ul li string,#stateList ul li string{padding: 5px 10px; background: #f0f0f0; display: block; width: 100%; font-weight:bold;}
		.badge-primary{background: var(--primary-color) !important; float:right;}
    </style>
                    ...
                    <div class="topbar-right">
                        ...
                        @if (is_plugin_active('real-estate') && RealEstateHelper::isRegisterEnabled())
                            <ul class="topbar-items">
                                ...
                                @else
                                    <li class="login-item">
										<a href="{{ route('public.account.login') }}"><i class="fas fa-sign-in-alt"></i>  {{ __('Login / Register') }}</a>
                                    </li>
                                @endif
                            </ul>
                            @if (auth('account')->check())
...
<header class="topmenu bg-light">
    ...
    @if (is_plugin_active('real-estate') && url()->current() == route('public.single') || ($page && $page->template === 'homepage'))
        @php
            $categories = get_property_categories(['indent' => '↳', 'conditions' => ['status' => \Botble\Base\Enums\BaseEnum::PUBLISHED]]);
        @endphp
            ...
            <div class="topsearch">
                ...
                <form @if (theme_option('enable_search_projects_on_homepage_search', 'yes') == 'yes') action="{{ route('public.projects') }}" @else action="{{ route('public.properties') }}" @endif method="GET" id="frmhomesearch">
                        <div class="typesearch" id="hometypesearch">
                            <a href="javascript:void(0)" class="active" rel="sale" data-url="{{ route('public.properties') }}">Buy</a>
							...
							@if (theme_option('enable_search_projects_on_homepage_search', 'yes') == 'yes')
                                <a href="javascript:void(0)"  rel="project" data-url="{{ route('public.projects') }}">{{ __('Projects') }}</a>
                            @endif
							<a href="javascript:void(0)" id="property-location-btn">Property Location</a>
                        </div>
                        <div class="input-group input-group-lg">
                            ...
                            <div class="advanced-search d-none d-sm-block">
                                <a href="javascript:void(0);" class="advanced-search-toggler">{{ __('Advanced') }} <i class="fas fa-caret-down"></i></a>
