    ...
    public function getProperties($filters = [], $params = [])
    {
        ...
        switch ($filters['sort_by']) {
            ...
            case 'name_desc':
                ...
                break;
			case 'is_featured_desc':
                $orderBy = [
                    're_properties.is_featured' => 'desc',
                ];
                break;
            default:
                $orderBy = [
        ...
        if ($filters['type'] !== null) {
            if ($filters['type'] == PropertyTypeEnum::SALE) {
                $this->model = $this->model->where('re_properties.type', $filters['type'])
                    ->whereIn('re_properties.status', [PropertyStatusEnum::SELLING,PropertyStatusEnum::SOLD]);
            } else {
                $this->model = $this->model->where('re_properties.type', $filters['type'])
                    ->whereIn('re_properties.status', [PropertyStatusEnum::RENTING,PropertyStatusEnum::RENTED]);
            }
        }
