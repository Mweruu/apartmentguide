...
use Botble\RealEstate\Services\SaveAvailabilitiesService;
...
class AccountPropertyController extends Controller
{
     ...
     * @param SaveAvailabilitiesService $saveAvailabilitiesService
    ...
    public function store(
        ...
        SaveAvailabilitiesService $saveAvailabilitiesService
    ) {
        if (!auth('account')->user()->canPost()) {
            return back()->with(['error_msg' => trans('plugins/real-estate::package.add_credit_alert')]);
        }
        ...
        if ($property) {
			...
			$saveAvailabilitiesService->execute($property, $request->input('availability', []));
